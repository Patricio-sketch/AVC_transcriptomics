...
│
├── rds/                              # Serialized R objects for session resumption (gitignored — regenerated by script.R)
...
This repository uses [Git LFS](https://git-lfs.github.com/) to store large model artifacts (`.safetensors`, `.pt`, `.bin`). Install and initialize LFS before cloning or pulling:
...
> If you cloned before installing LFS, run `git lfs pull` to download model files.
...
#### Reproducible environment with renv (recommended)

A `renv.lock` file is included to pin exact package versions. To restore the identical environment:
...
This is the recommended approach for exact reproducibility. The manual `BiocManager::install()` instructions above remain valid for quick setup without version pinning.
...
This repository uses [pre-commit](https://pre-commit.com/) hooks to prevent accidental leakage of secrets (HuggingFace tokens, API keys) and large notebook outputs.
...
- Strip all Jupyter notebook cell outputs and execution counts (`nbstripout`)
- Scan for private keys and high-entropy strings (`detect-secrets`)

> If you skip this setup, review cell outputs manually before committing notebooks, especially the `HF_TOKEN` variable in `clinicalbert_vias_AVC.ipynb`.
